...
export default {
    ...
    methods: {
        openModalIndicators(indicators) {
            this.$emit("openIndicatorsModal", indicators);
        },
        startCron() {
        ...
        updateTask(taskId, taskStatus, newStatus) {
            ...
            var taskSelected = dragAreasByStatus[taskStatus].find(
                (task) => task.id == taskId
            );

            var taskSelectedIndex = dragAreasByStatus[taskStatus].findIndex(
                (task) => task.id == taskId
            );
      ...
      this.updateStatus(taskId, newStatus) */
        ...
        updateTaskRealTime(taskToUpdate) {
            var taskSelected = this.tasks.find(
        ...
        incrementBar() {
            this.doneBar =
                this.doneBar +
                (this.toDo.length + this.doing.length + this.done.length) / 80;
        },
        ...
        addPercent() {
            ...
            this.percentAcad =
                this.percentAcad + (80 / this.tasks.length) * nroTasksDone;
        },
        getProjectById() {
            this.$swal({
                title: "Cargando ...",
            ...
            axios
                .get(`/api/projects/${this.$route.params.idProject}`)
                .then((res) => {
                    this.tasks = [];
        ...
        updateSinceRealTime(task) {
            ...
            var taskInToDo = this.toDo.find(
                (activity) => activity.id == task.activity_id
            );
            var taskInDoing = this.doing.find(
                (activity) => activity.id == task.activity_id
            );
    ...
    computed: {
        secondsFormatted() {
            if (this.totalTime >= 3600) {
                ...
                return (
                    totalHours + "h " + restMinutes + "m " + restSeconds + "s"
                );
