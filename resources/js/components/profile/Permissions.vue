<template>
    <h4>Permiso</h4>
    <div class="alert alert-warning" role="alert" v-if="permissionsNumber < 2">
        Ten cuidado antes de solicitar un permiso, ya que solo se pueden hacer
        dos solicitudes al mes. Ya hiciste {{ permissionsNumber }} este mes
    </div>
    <div class="alert alert-danger" role="alert" v-if="permissionsNumber >= 2">
        No puedes solicitar mas permisos este mes. Ya hiciste
        {{ permissionsNumber }}.
    </div>
    <div class="row">
        <div class="col-md-12">
        ...
        <div class="col-md-6 mt-3">
            ...
            <input
                ...
                @change="checkDatePermission"
                class="form-control"
            />
            <label for="">Hora de inicio</label>
            <input
                type="time"
                v-model="miss_time_admission"
                class="form-control"
            />
            <label for="">Hora de fin</label>
            <input
                type="time"
                v-model="miss_time_departure"
                @change="checkMissTimePermission"
                class="form-control"
            />
    ...
    <div class="row mt-3">
        <button
            ...
            v-bind:disabled="permissionsNumber >= 2"
        >
            Solicitar
        </button>
    </div>
    ...
    <div class="row">
        <div class="col">
            ...
            <input
                type="date"
                @change="testDateJustification"
                v-model="justification.miss_date"
                class="form-control"
            />
        </div>
        <div class="col">
            ...
            <input
                type="time"
                v-model="justification.admission_time"
                class="form-control"
            />
        </div>
        <div class="col">
            ...
            <input
                type="time"
                v-model="justification.departure_time"
                class="form-control"
            />
        </div>
    </div>
    ...
    <div class="row">
        <button class="btn btn-success mt-3" @click="postJustification">
            Enviar
        </button>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        checkDatePermission() {
            ...
            console.log(moment(datePicked).diff(actualDate));

            if (moment(datePicked).diff(actualDate) >= -18000000) {
        ...
        postPermission() {
            console.log(
                this.differenceTimesMiss == this.totalRecoveryMinutes,
                this.differenceTimesMiss,
                this.totalRecoveryMinutes
            );
            if (this.differenceTimesMiss == this.totalRecoveryMinutes) {
                ...
                axios
                    .post("/api/attendance-permits", fd)
                    .then((result) => {
                        ...
                        this.reason = "";
                        this.datePermission = "";
                        (this.dateRecovery = ""), (this.recoveries = []);
                ...
                this.$swal(
                    "Los tiempos de permiso no coinciden o ha superado el número de permisos de este mes"
                );
            }
        },
    },
    computed: {
        totalRecoveryMinutes() {
            ...
            var totalMinutesRecovery = this.recoveries.reduce(
