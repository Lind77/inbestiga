                ...
                <tbody v-if="order && order.quotation">
                    <tr class="text-dark" v-for="(detail, index) in order.quotation.details" :key="index">
                        ...
                        <td class="table-item" width="10%" v-if="detail.type <= order.suggested">
                            <p class="mb-0" style="text-align: center;">S./ {{ detail.price +
                                detail.extra_price ? detail.extra_price : 0 }}</p>
                        </td>
        ...
        totalPrice() {
            ...
            details.forEach(detail => {
                if (detail.type <= this.order.suggested) {
                    sumTotal += parseFloat(detail.price) + detail.extra_price
                }

            })

            return (sumTotal - parseFloat(this.order.quotation.discount)).toFixed(1)
        }
    }
}
</script>
