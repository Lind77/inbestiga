<template>
    <div class="cardSpace" draggable="true" @dragover.prevent @drop.stop.prevent @dragstart="drag" :id="`${customer.id}`"
        @click="showModalFunnel(customer)">
        <div :class="`card bg-${bgColor} p-2 cursor-pointer`">
            <template v-if="customer.customers">
                <h6 class="mb-0 py-2 text-white" v-for="customer in customer.customers">{{ customer.name || customer.cell }}
                </h6>
            </template>
            <template v-else>
                <h6 class="mb-0 py-2 text-white">{{ customer.name || customer.cell }}
                </h6>
            </template>
        </div>
