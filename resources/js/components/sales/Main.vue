                        ...
                        <div class="col-sm-7">
                            <div class="card-body">
                                ...
                                <p class="mb-4">
                                    Tienes
                                    <span class="fw-bold">{{
                                        comunications.length
                                    }}</span>
                                    comunicaciones pendientes para hoy, te
                            ...
                            >
                                ...
                                <div class="timeline-event">
                                    <div class="timeline-header mb-1">
                                        <h6 class="mb-0">
                                            {{ comunication.customer.name }} -
                                            {{ comunication.customer.cell }}
                                        </h6>
                                        <small
                                        ...
                                        <input
                                            v-show="showDatePicker"
                                            v-model="newDateTime"
                                            @blur="saveNewDate(comunication.id)"
                                            type="datetime-local"
    ...
    methods: {
        saveNewDate(comunicationId) {
            if (this.newDateTime != "") {
                ...
                axios
                    .post(
                        "/api/customers/next-comunication/" + comunicationId,
                        fd
                    )
                    .then((res) => {
                        var comunicationSelected = this.comunications.find(
                            (comunication) => comunication.id == comunicationId
                        );
                        comunicationSelected.next_management =
                            res.data.newComunication.next_management;
                        comunicationSelected.time =
                            res.data.newComunication.time;
        ...
        changeAtended(comunicationId) {
            if (confirm("¿Deseas marcar esta comunicación?")) {
                axios
                    .get("/api/updateComunication/" + comunicationId)
                    .then((res) => {
                        var comunicationSelected = this.comunications.find(
                            (comunication) => comunication.id == comunicationId
                        );
        ...
        formatTime(time, date) {
            ...
            var formatedTime = moment(time, "HH:mm:ss").format(
                "DD/MM/YYYY hh:mm a"
            );
            return formatedTime;
        },
        getComunicationsByToday() {
            axios
                .get("/api/getComunicationsByToday/" + this.store.authUser.id)
                .then((res) => {
        ...
        verifyComunication() {
            ...
            var modifiedTime = moment(timeSeconds, "HH:mm")
                .add(1, "hour")
                .format("HH:mm");

            var comunicationPlusHourFound = this.comunications.find(
                (comunication) => comunication.time == modifiedTime + ":00"
            );
            ...
            var comunicationFound = this.comunications.find(
                (comunication) => comunication.time == timeSeconds
            );

            if (
                comunicationFound &&
                this.store.authUser.roles[0].name == "Seller"
            ) {
                var nameAndCell =
                    comunicationFound.customer.name +
                    " - " +
                    comunicationFound.customer.cell;
                this.$swal(
                    "Usted tiene una comunicación pendiente en estos momentos con " +
                        nameAndCell
                );
